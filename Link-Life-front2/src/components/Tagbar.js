...
const Tagbar = ({ tagbarToggle }) => {
  return (
    <div
      className={`${tagbarToggle ? " hidden " : " block "}w-64 h-screen px-2 py-0 bg-green-100 m-5 border border-green-950 border-4 rounded`}
    >
      <div className="my-4 mb-4">
        ...
        <span className="relative inset-y-0 left-0 flex items-center w-full p-1 bg-gray-400 rounded">
          <input
            ...
            placeholder="해시태그 검색"
            value="#"
            className="hidden w-full px-4 py-1 bg-gray-400 rounded outline-none md:block"
          />
